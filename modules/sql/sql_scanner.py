...
class SQLScanner:
    ...
    def _scan_url(self, url):
        ...
        # For POST requests, use provided data
            ...
            # Parse POST data
            try:
                # Try to parse as application/x-www-form-urlencoded
                params = {}
                for param_pair in self.data.split('&'):
                    if '=' in param_pair:
    ...
    def _test_parameters(self, url, params, method):
        ...
        for param_name, param_values in params.items():
            param_value = param_values[0] if isinstance(
    ...
    def _test_time_based(self, url, param_name, params, method, baseline_response):
        ...
        for i, payload in enumerate(payloads):
            ...
            try:
                ...
                # Determine expected sleep time from the payload
