...
class ResponseAnalyzer:
    ...
    def _check_sql_errors(self, response_text):
        ...
        for pattern in self.mysql_error_patterns:
            ...
            if match:
                ...
                if self.verbose:
                    print(f"[+] MySQL error detected: {match.group(0)}")
    ...
    def _extract_data_from_error(self, response_text):
    ...
    def _check_union_select(self, response_text, payload):
        ...
        if not re.search(r"union\s+(?:all\s+)?select", payload, re.IGNORECASE):
            return False, None
    ...
    def _extract_data_from_union(self, response_text, payload):
        ...
        numeric_markers = re.findall(r"<td>\s*(\d+)\s*</td>", response_text)
        if numeric_markers:
